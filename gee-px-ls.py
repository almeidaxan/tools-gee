...
# ------------------------------------------------------------------------------------
# Description:
# 	- Download Landsat pixel time series (for the whole period available)
#	- Output is a .csv with each column containing a band
# ------------------------------------------------------------------------------------

# TODO arrumar if/elses da montagem do CSV !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
if(args.satprod == 'SR_old'):
	sat = satsSR_old[satsNames.index(args.satellite)]
if(args.satprod == 'SR_new'):
	sat = satsSR_new[satsNames.index(args.satellite)]
if(args.satprod == 'TOA'):
...
os.chdir(folName)
if not(os.path.exists(sat.replace('/', '-'))):
	os.mkdir(sat.replace('/', '-'))
os.chdir('..'+os.sep)
...
with open(csvName, 'rb') as csvfile:
	...
	for row in csvreader:
		plotName = row[0]
		arq = folName+os.sep+sat.replace('/', '-')+os.sep+plotName+'.csv'
		arqPrint = folName+os.sep+sat.replace('/', '-')+os.sep+(plotName+'.csv').ljust(maxLen+4)
		...
		if cond: # if cond and os.stat(arq).st_size>20:
			...
			if suc:
				with open(arq, 'wb') as csvfile:
					...
					if args.satellite == 8:
						if(args.satprod == 'SR_new'):
							csvwriter.writerow(['date', 'B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B10', 'B11', 'ndvi', 'evi', 'evi2'])
						else:
